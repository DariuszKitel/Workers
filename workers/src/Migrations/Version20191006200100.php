 ...
 */
final class Version20191006200100 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, password, name, email, pass_change_date FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, name VARCHAR(255) NOT NULL COLLATE BINARY, email VARCHAR(255) NOT NULL COLLATE BINARY, pass_change_date INTEGER DEFAULT NULL, enabled BOOLEAN DEFAULT \'0\' NOT NULL, confirmation_token VARCHAR(40) DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, username, password, name, email, pass_change_date) SELECT id, username, password, name, email, pass_change_date FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, password, name, email, pass_change_date FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, name VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, pass_change_date INTEGER DEFAULT NULL, roles CLOB DEFAULT \'ROLE_QUESTIONER\' NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO user (id, username, password, name, email, pass_change_date) SELECT id, username, password, name, email, pass_change_date FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('DROP INDEX IDX_A349DDB5F675F31B');
        $this->addSql('CREATE TEMPORARY TABLE __temp__work_post AS SELECT id, author_id, title, published, content, cv, slug FROM work_post');
